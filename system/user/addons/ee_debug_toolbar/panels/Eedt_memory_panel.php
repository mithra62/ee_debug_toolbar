...
class Eedt_memory_panel extends Eedt_base_panel
{
    ...
    {
        parent::__construct();
        $this->button_label = ee()->toolbar->filesize_format(memory_get_peak_usage()) . ' ' . ini_get('memory_limit');
    }
    ...
    public function ee_debug_toolbar_add_panel($view)
    {
        ...
        $view->add_css(ee()->toolbar->create_theme_url('default', 'css') . '/ee_debug_panel_memory.css');
        $view->add_js(ee()->toolbar->create_theme_url('default', 'js') . '/ee_debug_panel_memory.js');
